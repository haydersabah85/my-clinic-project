...
if (isset($_POST['id'])) {
    ...
    if ($appointment_id > 0) {
        $update_surgery_query = "UPDATE surgery_appointment SET status = 'discharged', updated_at = NOW() $syncPart WHERE id = '$appointment_id' AND patient_id = '$id'";
    } else {
        $update_surgery_query = "UPDATE surgery_appointment SET status = 'discharged', updated_at = NOW() $syncPart WHERE patient_id = '$id'";
    }
    mysqli_query($con, $update_surgery_query);
    ...
    if ($row_patient && $no_show_reason !== '') {
        ...
        $noShowNote = "[لم يحضر موعد العملية {$noteDate}] " . $no_show_reason;
