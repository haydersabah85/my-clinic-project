...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        h1 { margin: 0; font-size: 26px; color: #0f5790; }
        ...
        .empty { padding: 14px; color: #64748b; background: #f8fafc; border-radius: 10px; }
        ...
        @media (max-width: 850px) {
            body { padding: 12px; }
            .topbar, .patient-row { grid-template-columns: 1fr; align-items: stretch; }
            .topbar { flex-direction: column; align-items: stretch; }
            .grid { grid-template-columns: 1fr; }
            .links a, .patient-row a, button { text-align: center; width: 100%; }
        }
    </style>
</head>
<body class="clinic-polished">
    <div class="page">
        <div class="topbar">
            <div>
                <h1>إعطاء موعد مراجعة</h1>
                <div class="muted">اختر أي مريض وأي تاريخ من أي يوم أو شهر، بدون حد أقصى لعدد المرضى في اليوم.</div>
            </div>
            <div class="links">
                ...
                <?php if ($history && mysqli_num_rows($history) > 0): ?>
                    <?php while ($row = mysqli_fetch_assoc($history)): ?>
                        <div class="history-item">
                            ...
                            <div class="muted"><?= htmlspecialchars($row['note'] ?: '', ENT_QUOTES, 'UTF-8') ?> | الحالة: <?= htmlspecialchars($row['status'], ENT_QUOTES, 'UTF-8') ?></div>
                        </div>
                    <?php endwhile; ?>
                <?php else: ?>
