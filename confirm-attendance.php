...
$update_surgical = "UPDATE surgery_appointment SET attendance_status = 1 WHERE id = ?";
$stmt_surgical = $con->prepare($update_surgical);
$stmt_surgical->bind_param("i", $id);
$stmt_surgical->execute();

$update_laser = "UPDATE laser_appointment SET attendance_status = 1 WHERE id = ?";
$stmt_laser = $con->prepare($update_laser);
$stmt_laser->bind_param("i", $id);
$stmt_laser->execute();

$update_injection = "UPDATE injection_appointment SET attendance_status = 1 WHERE id = ?";
