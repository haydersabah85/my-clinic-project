...
$injection_types = clinic_get_injection_types($con);
if (empty($injection_types)) {
  $injection_types = ['Avastin', 'Eylea 2mg', 'Vabysmo', 'Eylea 8mg', 'Triamcinolone', 'Ozurdix'];
}

if (!$injection_row) {
  http_response_code(404);
  exit('Injection record not found.');
}
...
<body>
  ...
  <form class="injection" action="edit-injection2.php?id_update= <?php echo $injection_row['id'] ?>" method="POST">
    ...
    <textarea
      ...
      cols="50"
      rows="5"
      placeholder="Extra Notes.."><?php echo htmlspecialchars($injection_row['notes']); ?></textarea>
